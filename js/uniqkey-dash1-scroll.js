/*
  Pinned scroll for the redesigned nav+dashboard pair (Uniqkey case,
  "Redesigned 1/2" section). Same technique as js/jobcard-scroll.js:
  the row sticks in place while the dashboard screenshot translates
  ...
  Unlike jobcard-scroll.js this is NOT gated to desktop — the row stays
  horizontal (menu + dashboard side by side) at every breakpoint, so the
  pin/reveal effect runs at every breakpoint too. Only
  prefers-reduced-motion disables it, falling back to a plain static
  row — still side by side, just with no pin and no translate.
...
*/
(function () {
  var spacer = document.querySelector('[data-dash1-pin]');
  var pinInner = document.querySelector('.case-dash1__row');
  var menu = document.querySelector('[data-dash1-menu]');
  var viewport = document.querySelector('[data-dash1-db-viewport]');
  var track = document.querySelector('[data-dash1-db-track]');
  ...
  function recompute() {
    if (nav) {
      document.documentElement.style.setProperty('--dash1-pin-top', nav.getBoundingClientRect().height + 'px');
    }
